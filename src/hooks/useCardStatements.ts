...
export const useCardStatements = (cardId?: string) => {
  ...
  const calculateStatementTotal = async (cardId: string, startDate: Date, endDate: Date): Promise<number> => {
    // Add 3 days margin to avoid timezone drift in SQL, filter exactly in JS
    const marginStart = new Date(startDate);
    marginStart.setDate(marginStart.getDate() - 3);
    const marginEnd = new Date(endDate);
    marginEnd.setDate(marginEnd.getDate() + 3);
    ...
    const { data, error } = await supabase
      ...
      .eq('card_id', cardId)
      .eq('type', 'expense')
      .gte('date', marginStart.toISOString())
      .lte('date', marginEnd.toISOString());

    if (error) throw error;
    
    // Exact Javascript filtering
  ...
  const getOrCreateStatement = useMutation({
    mutationFn: async ({ cardId, month, closingDay = 1, dueDay = 10 }: { 
      ...
      // Calculate total from transactions using closing cycle
      const cycleStart = new Date(referenceMonth.getFullYear(), referenceMonth.getMonth() - 1, closingDay + 1);
  ...
  const updateStatementTotal = useMutation({
    mutationFn: async (statementId: string) => {
      ...
      const closingDate = new Date(stmnt.closing_date);
      
      const cycleStart = new Date(closingDate.getFullYear(), closingDate.getMonth() - 1, closingDate.getDate() + 1);
      cycleStart.setHours(0, 0, 0, 0);
      const cycleEnd = new Date(closingDate);
      cycleEnd.setHours(23, 59, 59, 999);
...
export const useStatementTransactions = (cardId: string, referenceMonth: Date, closingDay = 1) => {
  const { user } = useAuth();
  ...
  return useQuery({
    ...
    queryFn: async () => {
      // Broaden SQL query by 3 days for DB timezone safety
      const marginStart = new Date(startDate);
      marginStart.setDate(marginStart.getDate() - 3);
      const marginEnd = new Date(endDate);
      marginEnd.setDate(marginEnd.getDate() + 3);
