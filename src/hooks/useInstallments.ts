    ...
    mutationFn: async ({
      ...
      const { error: debitError } = await supabase.from("transactions").insert({
        ...
        description: installments.length > 1 
          ? `🎯 Antecipação Estratégica: ${description} (${installments.length} parcelas)`
          : `🎯 Antecipação Estratégica: ${description} (1 parcela)`,
        amount: finalAmount,
        type: "expense",
      ...
      return { success: true, count: installments.length, total: finalAmount };
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["installment-transactions"] });
      queryClient.invalidateQueries({ queryKey: ["transactions"] });
