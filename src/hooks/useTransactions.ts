...
export const useTransactions = (filters?: TransactionFilters) => {
  ...
  const { data: transactions = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters?.startDate) {
        ...
        query = query.gte('date', `${dateStr}T00:00:00.000Z`);
      }
      if (filters?.endDate) {
        const d = filters.endDate;
        const dateStr = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
        query = query.lte('date', `${dateStr}T23:59:59.999Z`);
      }
      if (filters?.bankId) {
    ...
    mutationFn: async (transaction: Omit<Transaction, 'id'>) => {
      ...
      if (validated.isInstallment && validated.installmentCount && validated.installmentCount > 1) {
        ...
        for (let i = 1; i <= validated.installmentCount; i++) {
          ...
          installments.push({
            ...
            date: `${installmentDate.getFullYear()}-${String(installmentDate.getMonth() + 1).padStart(2, '0')}-${String(installmentDate.getDate()).padStart(2, '0')}T00:00:00.000Z`,
            is_installment: true,
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          bank_id: validated.bankId,
          card_id: transaction.cardId,
          date: `${validated.date.getFullYear()}-${String(validated.date.getMonth() + 1).padStart(2, '0')}-${String(validated.date.getDate()).padStart(2, '0')}T00:00:00.000Z`,
  ...
  const addBatchTransactions = useMutation({
    mutationFn: async (batchData: { transactions: BatchTransaction[], categories: any[], banks: any[], globalCardId?: string }) => {
      if (!user) throw new Error('User not authenticated');

      const { transactions: batchTransactions, categories, banks, globalCardId } = batchData;
  ...
  const addBulkSimpleTransactions = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['transactions'] });
      queryClient.invalidateQueries({ queryKey: ['banks'] });
      toast.success(`${data.count} transações adicionais registradas!`);
    },
    onError: (error: any) => {
      toast.error(`Erro ao adicionar transações: ${error.message}`);
    },
  });
  ...
  const deleteTransaction = useMutation({
    mutationFn: async (id: string) => {
      ...
      if (txData?.is_installment && txData?.installment_number === 1 && txData?.card_id && txData?.installment_count) {
        // Get all related installments to sum their actual amounts
        const { data: relatedTxs } = await supabase
          .from('transactions')
          .select('amount')
          .eq('parent_transaction_id', id);
        
        const totalAmount = Number(txData.amount) + (relatedTxs?.reduce((sum, tx) => sum + Number(tx.amount), 0) || 0);
  ...
  return {
    ...
    addBulkSimpleTransactions: addBulkSimpleTransactions.mutateAsync,
  };
};
