...
export const useLoans = () => {
  ...
  const payMultipleLoanInstallments = useMutation({
    mutationFn: async ({
      loanId,
      installmentIds,
      bankId,
      ...
      const targetPayments = loan.payments.filter((p) => installmentIds.includes(p.id));
      if (targetPayments.length === 0) throw new Error("Parcelas não encontradas");

      const totalAmount = targetPayments.reduce((sum, p) => sum + p.amount, 0);
      const finalAmount = totalAmount - discount;
      ...
      if (createTransaction && transactionBankId && user) {
        ...
        if (categoryId) {
          await supabase.from("transactions").insert({
            user_id: user.id,
            description: targetPayments.length > 1
              ? `🎯 Antecipação Estratégica: ${loanData.name} (${targetPayments.length} parcelas)`
              : `🎯 Antecipação Estratégica: ${loanData.name} (1 parcela)`,
            amount: finalAmount,
            type: "expense",
            category_id: categoryId,
            subcategory: loanData.subcategory,
      ...
      return { count: targetPayments.length, totalAmount: finalAmount };
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["loans"] });
      queryClient.invalidateQueries({ queryKey: ["transactions"] });
    ...
    onError: (error: any) => {
      toast.error(`Erro ao pagar parcelas: ${error.message}`);
    },
  });
