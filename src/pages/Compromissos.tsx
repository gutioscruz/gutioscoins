...
const Compromissos = () => {
  ...
  const CommitmentCard = ({ commitment }: { commitment: Commitment }) => {
    ...
    return (
      <div className="group rounded-3xl bg-card/60 backdrop-blur-md border border-white/5 shadow-sm p-4 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] hover:-translate-y-1 transition-all duration-500 ease-out">
        <div className="flex items-start gap-4">
          <div className="p-2.5 rounded-xl bg-muted/80 shrink-0 mt-0.5 group-hover:bg-primary/10 transition-colors">
      ...
      <main className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {[
            ...
            <div 
              key={card.label} 
              className="group rounded-3xl bg-card/60 backdrop-blur-md border border-white/5 shadow-sm p-5 hover:bg-card/80 transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] hover:-translate-y-1"
              style={{ animationDelay: `${idx * 100}ms` }}
            >
              <p className="text-xs text-muted-foreground font-medium mb-1 group-hover:text-primary transition-colors">{card.label}</p>
          ...
          <TabsContent value="parcelamentos" className="mt-4 space-y-3">
            {installmentCommitments.length === 0 ? (
              <PremiumEmptyState 
                ...
                subtitle="Nenhum parcelamento ativo no momento. Foco na dieta e no treino!" 
              />
            ) : (
          ...
          <TabsContent value="emprestimos" className="mt-4 space-y-3">
            {loanCommitments.length === 0 ? (
              <PremiumEmptyState 
                ...
                subtitle="Nenhum empréstimo ativo. Continue construindo seu patrimônio." 
              />
            ) : (
