      ...
      <div className="space-y-3">
        ...
        <div className="flex justify-between text-xs">
          <span className="text-muted-foreground">
            Guardado: <span className={`font-semibold ${data.isOnTrack ? "text-income" : "text-foreground"}`}>{formatCurrency(data.saved)}</span>
          </span>
          <span className="text-muted-foreground">
            Meta: <span className="font-semibold text-foreground">{formatCurrency(data.targetAmount)}</span>
          </span>
        </div>
