...
export const MonthlyStatementsOverview = ({ banks, onCardClick }: MonthlyStatementsOverviewProps) => {
  ...
  const { data: realTimeAmounts = {}, isLoading: isTransactionsLoading } = useQuery({
    queryKey: ['monthly-realtime-amounts', format(currentMonth, 'yyyy-MM'), cardIds],
    queryFn: async () => {
      if (cardIds.length === 0) return {};
  ...
  const isLoading = isStatementsLoading || isTransactionsLoading;

  const statements: StatementData[] = useMemo(() => {
    return allCards.map(card => {
      const statement = statementsData.find(s => s.card_id === card.id);
      const computedAmount = realTimeAmounts[card.id] || 0;
      
      const isSavedAndFinal = statement && (statement.status === 'closed' || statement.status === 'paid');
      const totalAmount = isSavedAndFinal ? Number(statement.total_amount) : computedAmount;
  ...
  }, [allCards, statementsData, realTimeAmounts]);
