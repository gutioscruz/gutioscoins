...
export const InteractivePieChart = ({
  ...
  if (chartData.length === 0) {
    return (
      <Card className="rounded-3xl bg-card/60 backdrop-blur-md border border-white/5 shadow-sm">
  ...
  return (
    <Card className="rounded-3xl bg-card/60 backdrop-blur-md border border-white/5 shadow-sm overflow-hidden">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-bold tracking-tight">{title}</CardTitle>
        <CardDescription className="text-xs">{description}</CardDescription>
      </CardHeader>
      <CardContent className="pb-4">
        ...
        <div className="space-y-1.5 mt-2 max-h-[180px] overflow-y-auto">
          {chartData.map((item, index) => (
            <button
              key={item.name}
              className="flex items-center justify-between w-full px-3 py-2 rounded-xl hover:bg-white/5 transition-all duration-300 text-left group/item"
